...
export default function ViewDevice(props) {
	...
	return (
		<>
			<DetailHeader title='機器詳細' buttons={<input type='submit' value='完了' className={styles.detailButton} onClick={() => saveRecord(props.saveUrl, formDate, props.setMode)} />} />
			<div className={styles.detailBody}>
				<table className={styles.editTable}>
					<tbody>
						...
						<tr><th>更新日</th><td><input type='date' name='updateDate' value={registerDate} onChange={(e) => setUpdateDate(e.target.value)} /></td></tr>
						<tr><th>備考</th><td><input type='text' name='remarks' value={remarks} onChange={(e) => setRemarks(e.target.value)} /></td></tr>
					</tbody>
				</table>
			</div>
			<DetailFooter buttons={<input type='button' value='削除' className={dangerButtonClass} onClick={() => deleteRecord(props.saveUrl, formDate, props.setMode, props.setActiveId)}/>} />
		</>
	)
}